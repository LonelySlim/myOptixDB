...
int get_iso_week_number(char* date_str) {
    ...
    // 生成时间戳
    time_epoch = mktime(&time_struct);

    // 重新填充 tm 结构
    localtime_r(&time_epoch, &time_struct);

    // ISO 周数计算，需要调整周数和年份的计算方式
    char buffer[32];
    strftime(buffer, 32, "%V", &time_struct);  // 使用 %V 获取 ISO 周数
...
void processCSVLine(char *line, uint32_t **outputdata_avg, uint32_t **outputdata_scan, uint32_t **outputdata_group, uint32_t &count) {
    ...
    for(uint32_t i = 0; i < SCAN_NUM; i++) {
        ...
        if(i == 0) {
            ...
            outputdata_scan[i][count] = get_iso_week_number(cleanedStr);
            outputdata_scan[i + 1][count] = ttoken / 10000;
        }
        else {
            uint32_t ttoken = atoi(token);
            outputdata_scan[i + 1][count] = ttoken;
        }
    }
...
int main(){
    ...
    outputdata_scan[3] = (uint32_t *)malloc(N * sizeof(uint32_t));
    ...
    inputfile = fopen("../q1dot1/file_q1dot1.csv", "r");
    ...
    outputfile1 = fopen("outputfile_rtscan_q1dot3.txt", "wb");
    ...
    fwrite(outputdata_scan[3], sizeof(uint32_t), N, outputfile);
    ...
    fwrite(outputdata_scan[3], sizeof(uint32_t), N, outputfile1);
    fclose(outputfile1);
