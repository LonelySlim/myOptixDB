...
void processCSVLine(char *line, uint32_t **outputdata_avg, uint32_t **outputdata_rtscan, uint32_t **outputdata_group, uint32_t &count) {
    ...
    for(uint32_t i = 0; i < SCAN_NUM; i++) {
        ...
        if(i == 2) {
            ...
            outputdata_rtscan[i][count] = ttoken / 100;
        }
        else {
            uint32_t tag = -1;
...
int main(){
    ...
    inputfile = fopen("../q3dot2/file_q3dot2.csv", "r");
    ...
    outputfile1 = fopen("outputfile_rtscan_q3dot4.txt", "wb");
    ...
    fwrite(outputdata_group[2], sizeof(uint32_t), N, outputfile1);
